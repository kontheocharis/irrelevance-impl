...
initialElabState = MkElabState (MkProject Nothing empty Nothing) False
...
printTermAnd expand b = do
  ...
  tc $ runAfter (\ctx, tm => do
      ...
      let term : Atom _ = if expand
                    then expandDefs ctx.scope.defs tm.tm
                    else tm.tm
...
printTypeAnd expand b = do
  ...
  tc $ runAfter (\ctx, tm => do
      ...
      let type : Atom _ = if expand
                    then expandDefs ctx.scope.defs tm.annot
                    else tm.annot
...
elab PUnit = tc $ tcPrim PrimTT []
elab (PSigmas (MkPTel [])) = tc $ tcPrim PrimUNIT []
...
elab (PBlock t (PLet l ((_, m), n) ty tm :: bs)) = enterLoc l $ do
  ty' <- traverse elab ty
  tm' <- elab tm
  bs' <- elab (PBlock t bs)
  tc (tcLet n (Just m) ty' tm' bs')
elab (PBlock t (PLetRec l ((_, m), n) ty tm :: bs)) = enterLoc l $ do
  ty' <- elab ty
  tm' <- elab tm
  bs' <- elab (PBlock t bs)
  tc $ tcLetRec n (Just m) ty' tm' bs'
elab (PBlock t (PBlockTm l tm :: [])) = do
  enterLoc l $ elab tm
elab (PBlock t (PDecl l ((_, m), n) ty :: bs)) = enterLoc l $ do
  p <- resetIsPrimitive
  if p
    then do
      ty' <- elab ty
      bs' <- elab (PBlock t bs)
      tc $ tcPrimDecl n (Just m) ty' bs'
    else elabError DeclNotSupported
elab (PBlock t (PBind l n ty tm :: bs)) = do
  ?todoBind
elab (PBlock t (PBlockTm l tm :: bs)) = do
  elab (PBlock t (PBind l ((Explicit, Unres), "_") Nothing tm :: bs))
