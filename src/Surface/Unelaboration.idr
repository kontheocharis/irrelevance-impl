...
export
Unelab (Binding r Syntax) PTm where
  unelab (Bound InternalLam y) = do
    n <- fresh
...
Unelab (Head Syntax NA) PTm where
  ...
  unelab (SynBinding _ b) = unelab b
...
{d : Domain} -> Unelab (Term d) PTm where
  ...
  unelab {d = Syntax} (RigidBinding b) = unelab b
...
[unelabContext] {onlyBinds : Bool} -> Unelab (Context bs) PBlockStatements where
  ...
  unelab (MkContext (Val (sx :< x)) (Val (bx :< x')) (tys :< ty) (MkScope (SS sb) (SS sn) (Lift zs)) bs)
    = do
  ...
  unelab (MkContext (Val (sx :< x)) (Val bx) (tys :< ty) (MkScope sb (SS sn) (zs :< z)) bs)
    = if onlyBinds
         then unelab (MkContext (Val sx) (Val bx) tys (MkScope sb sn zs) dummy)
         else do 
          MkPBlockStatements rest <- unelab (MkContext (Val sx) (Val bx) tys (MkScope sb sn zs) dummy)
          ty' <- unelab ty
          tm' <- unelab z
...
[unelabGoal] Unelab (\_ => Goal) PGoal where
  unelab (MkGoal name hole ctx) = do
    ...
    ty' <- unelab hole.annot
    pure $ MkPGoal ctx' (maybe (PName "?_") (\n => PName "?\{n}") name) ty'
